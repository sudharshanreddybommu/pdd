...
import { getDoctorAppointments, scheduleAppointment, getNotifications, markNotificationsRead } from '../../services/api'
...
export default function DoctorDashboard() {
  ...
  const tabData = { pending, scheduled, all: appointments }
    ...
    <div className="page fade-in">
      ...
      <div className="container" style={{ paddingTop: 40, paddingBottom: 60 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          {[['pending', `⏳ Pending (${pending.length})`], ['scheduled', `✅ Scheduled (${scheduled.length})`], ['all', `📋 All (${appointments.length})`], ['notifications', `🔔 Notifications`]].map(([key, label]) => (
                ...
                <div className="appt-card" key={appt.id}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <div className={`status-badge status-${appt.status}`}>{STATUS_LABEL[appt.status]}</div>
                  ...
                  {isOpen && (
                    <div style={{ marginTop: 20 }}>
                      ...
                      {appt.scheduled_date && (
                        <div className="alert alert-success mb-2"><span>📅</span><span>Scheduled: <strong>{new Date(appt.scheduled_date).toLocaleString()}</strong></span></div>
                      )}
                      {appt.notes && (
                      ...
                      {appt.status === 'pending' && (
                        scheduling === appt.id ? (
                          ...
                          <button onClick={() => setScheduling(appt.id)} className="btn btn-primary">📅 Schedule Appointment</button>
                        )
                      )}
                    </div>
                  )}
